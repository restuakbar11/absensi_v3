...
$ukuran_file  = $_FILES['webcam']['size'];
$filename =''.$date.'-in-'.time().'-'.$row_user['id'].'.jpeg';
$directory= "../sw-content/lembur/".$filename;
if (empty($_GET['latitude'])) {
      $error[] = 'tidak boleh kosong';
    } else {
      $latitude= mysqli_real_escape_string($connection, $_GET['latitude']);
...
$query  ="SELECT presence_date,employees_id,time_in,time_out FROM presence_lembur 
WHERE employees_id='$row_user[id]' AND DATEDIFF(CURRENT_DATE(), presence_date) <= '1' AND time_out = '00:00:00'";
$result = $connection->query($query);
...
if ($result->num_rows > 0) {
    $filename =''.$date.'-out-'.time().'-'.$row_user['id'].'.jpeg';
    $directory= "../sw-content/lembur/".$filename;
    /* -------- Upload Foto Selesai Lembur -------*/
    $update ="UPDATE presence_lembur SET time_out='$time',picture_out='$filename',latitude_longtitude_out='$latitude' WHERE employees_id='$hasil[employees_id]' AND DATEDIFF(CURRENT_DATE(), presence_date) <= '1'";
    if($connection->query($update) === false) { 
        die($connection->error.__LINE__); 
        echo'Sepetinya sitem kami sedang error!';
    } else{
          //Selesai Lembur
        echo'success/Selamat "'.$row_user['employees_name'].'" berhasil Absen Selesai Lembur pada Tanggal '.tanggal_ind($date).' dan Jam : '.$time.', Semoga lembur hari ini berkah "'.$row_user['employees_name'].'"!';
        //imagejpeg($tmp_name,$directory,80);
    }
  }else{
    $add ="INSERT INTO presence_lembur (employees_id,presence_date,time_in,time_out,picture_in,picture_out,present_id,
          latitude_longtitude_in,latitude_longtitude_out,information)
    values('$row_user[id]','$date','$time','00:00:00','$filename','','1','$latitude','','')";
      if($connection->query($add) === false) { 
          die($connection->error.__LINE__); 
          echo'Sepertinya Sistem Kami sedang error!';
      } else{  
          ...
          //imagejpeg($tmp_name,$directory,80);
          move_uploaded_file($_FILES['webcam']['tmp_name'], $directory);
      }
}
